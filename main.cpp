...
//
//     Device 389003            "Chipkin Example B-ASC"     (instance configurable with --deviceID)
...
// Object_Name "Chipkin Example B-ASC" - a spec violation, and a hard BTL failure. In a real
...
static const char* DEVICE_NAME = "Chipkin Example B-ASC";
...
// (no password required). Change this to your device's secret before shipping - and
// remember it crosses the wire in PLAINTEXT: it is a guard against accidents, not a
// security boundary. See the DeviceCommunicationControl callback below for the compare.
...
// The device instance below (g_deviceInstance, default 389003, --deviceID overrides)
// must also be unique on the internetwork; BACnet requires it to be configurable, so
// keep the --deviceID override rather than hard-coding it.
